...
public class Tilemap
{
    ...
    /// <summary>
    /// Gets the total number of rows in this tilemsp.
    /// </summary>
    public int Rows { get; }
    ...
    /// <summary>
    /// Gets the total numberof tiles in this tilemsp.
    /// </summary>
    ...
    public static Tilemap FromFile(ContentManager content, string filename)
    {
        ...
        using (Stream stream = TitleContainer.OpenStream(filePath))
        {
            using (XmlReader reader = XmlReader.Create(stream)) 
            {
                ...
                int colunmCount = rows[0].Split(" ", StringSplitOptions.RemoveEmptyEntries).Length;

                // Create the tilemap
                Tilemap tilemap = new Tilemap(tileset, colunmCount, rows.Length);
